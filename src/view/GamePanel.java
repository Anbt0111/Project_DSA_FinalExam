...
public class GamePanel extends JPanel implements Runnable {
    ...
    public GamePanel(View view) {
        ...
        exitButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), exitButton.getName());
        exitButton.getActionMap().put(exitButton.getName(), view.actionController);
        this.add(exitButton);
        ...
        removeButton.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_BACK_SPACE, 0), removeButton.getName());
        removeButton.getActionMap().put(removeButton.getName(), view.actionController);
    ...
    private void addSymbolTable() {
        ...
        for (int i = 1; i <= 6; i++) {
            ...
            symbol.setBounds(
                    ...
                    View.GRID_WIDTH,
                    View.GRID_HEIGHT);
            symbol.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(48 + i, 0), symbol.getName()); // key code for 1 to 6 is 49 to 54
            symbol.getActionMap().put(symbol.getName(), view.actionController);
    ...
    private void attackPhase() {
        if (!timer.isCounting()) { // end of attack phase, back to memorize phase
            ...
            if (gojo.getHealth() == 0) {
                view.updateScore();
                view.changePanel(View.END_SCREEN);
                stop();
            }
    ...
    private void castingPhaseSetup() {
        ...
        timer.countdown(Countdown.MIN_INPUT_TIME + 4 / turn);
    }
